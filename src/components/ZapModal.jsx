...
export function ZapModal() {
  ...
  useEffect(() => {
    const handleGlobalClick = (e) => {
      ...
      if (btn) {
        ...
        const customMessage = btn.getAttribute('data-message') || DEFAULT_MESSAGE;
  ...
  return (
    ...
    >
      <div onClick={closeModal} className="absolute inset-0 bg-zinc-950/80 backdrop-blur-sm"></div>

      <div
        className={`relative z-10 w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-2xl transition-transform duration-300 ${
          isOpen ? 'translate-y-0 scale-100' : 'translate-y-4 scale-95'
        }`}
      >
        <div className="mb-5 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <svg className="h-5 w-5 shrink-0 text-emerald-500" fill="currentColor" viewBox="0 0 24 24">
            ...
            <h2 id="modal-title" className="text-sm font-bold tracking-widest text-zinc-300 uppercase">
              Enviar mensagem
            </h2>
          </div>
          <button onClick={closeModal} aria-label="Fechar" className="rounded p-1 text-zinc-500 transition-colors hover:text-zinc-200">
            <svg className="h-5 w-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <div className="mb-6 rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3">
          <p className="text-sm leading-relaxed text-zinc-300">{message}</p>
        </div>

        <div className="flex gap-3">
          <button onClick={closeModal} className="flex-1 rounded-lg border border-zinc-700 px-4 py-3 text-sm font-bold tracking-wider text-zinc-400 uppercase transition-colors hover:bg-zinc-800 hover:text-zinc-200">
            Cancelar
          </button>
          <a
            href={whatsappUrl}
